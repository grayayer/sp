	...
	function widget( $args, $instance ) {  
		...
		if ( count( $events ) > 0 ) {
			...
			foreach ( $events as $k => $post ) {
				...
				$html .= '<td><p class="event-title"><a href="' . get_permalink( $post->ID ) . '">' . get_the_title( $post->ID ) . '</a></p><p class="event-widget-venue">' . $venue . '</p></td>' . "\n";
...
?>
       ...
       <p>
	   	   <label for="<?php echo $this->get_field_id( 'limit' ); ?>"><?php _e( 'Limit (default: 5):', 'woothemes' ); ?></label>
